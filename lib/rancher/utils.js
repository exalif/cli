...
module.exports = class RancherUtils {
  ...
  static getServiceUpgradePayload(targetService) {
    ...
    const completeDeploymentTarget = [
      this.type === 'deployment'
        ? `deployment:${targetService}`
        : this.type === 'statefulset'
          ? `statefulset:${targetService}`
          : targetService
    ];
  ...
  static checkDeployOnCluster(targetService) {
    ...
    const { checks: configChecks, maxCheckRetries: maxRetries, initialCheckWaitDelay: initialWaitDelay} = config[this.type];

    let conditions = '';
    let checks = { ...configChecks };

    if (this.assertAgainstTemplateReplicas && this.replicas && Number(this.replicas) > 0 && config.checkReplicas.find(check => check.type === this.type)) {
      const { replicasStatusKey } = config.checkReplicas.find(check => check.type === this.type);

      const { [replicasStatusKey]: oldStatusKey, ...otherChecks } = checks;

      checks = {
        ...otherChecks,
        [replicasStatusKey]: this.replicas,
      };
    }
