...
module.exports = class RancherUtils {
  ...
  static checkDeployOnCluster(targetService) {
    ...
    config[this.type].checkKeys.forEach((key, i) => {
      conditions += '\n  ' + key + ' = ' + config[this.type].expectedCheckValues[i];
    });
    ...
    return retryTaskUntilExpectedValue({
      ...
      expectedValues: config[this.type].expectedCheckValues,
      maxRetries: config[this.type].maxCheckRetries,
      initialWaitDelay: config[this.type].initialCheckWaitDelay,
    });
  }
